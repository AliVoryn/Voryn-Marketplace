...
contract AuctionMathTest is Test {
    ...
    function testFuzz_shouldExtend_NeverExtendsPastMax(
        uint64 nowTs,
        uint64 endTime,
        uint64 window,
        uint8 used,
    ...
    function test_shouldExtend_FalseExactlyAtEndTime() public view {
        assertFalse(AuctionMath.shouldExtend(uint64(block.timestamp), uint64(block.timestamp), 10, 0, 3));
    }

    function test_shouldExtend_TrueExactlyAtWindowBoundary() public view {
        assertTrue(AuctionMath.shouldExtend(uint64(block.timestamp), uint64(block.timestamp + 10), 10, 0, 3));
    }

    function test_shouldExtend_FalseOneUnitBeyondWindow() public view {
        assertFalse(AuctionMath.shouldExtend(uint64(block.timestamp), uint64(block.timestamp + 11), 10, 0, 3));
    }

    function test_shouldExtend_TrueOneBelowMaxExtensions() public view {
        assertTrue(AuctionMath.shouldExtend(uint64(block.timestamp), uint64(block.timestamp + 5), 10, 2, 3));
    }
}
