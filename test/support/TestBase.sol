...
abstract contract ProtocolTestBase is Test {
    ...
    function _deployTreasury() internal returns (Treasury t) {
        t = new Treasury(admin, feeRecipient, address(0));
    }

    function _deployPaymentManager() internal returns (PaymentManager pm) {
        pm = new PaymentManager(admin, address(0));
    }
    ...
    function _deployRaffle(address treasury_, MockVRFCoordinatorV2Plus coordinator) internal returns (Raffle rf) {
        rf = new Raffle(admin, treasury_, FEE_BPS, address(coordinator), 1, bytes32(uint256(1)), 200_000, 3, false);
    }
