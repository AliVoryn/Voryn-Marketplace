...
contract ProtocolFactory is Ownable2Step, IFactory {
    ...
    function createProtocolSuite(address admin, address feeRecipient)
        external
        override
        returns (address treasury, address paymentManager, address marketplace, address auction, address staking)
    {
        ...
        DutchAuction dutchAuctionContract = new DutchAuction(admin, address(treasuryContract), 250);
        ...
        treasuryContract.setAuthorizedPayer(address(dutchAuctionContract), true);
        ...
        _register(address(dutchAuctionContract), msg.sender, address(0), KIND_DUTCH_AUCTION);
        ...
        emit ProtocolSuiteDutchAuctionCreated(msg.sender, treasury, address(dutchAuctionContract));
    }
