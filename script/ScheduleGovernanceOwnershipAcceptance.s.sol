...
interface IOwnershipAcceptance {
    function acceptOwnership() external;
}

contract ScheduleGovernanceOwnershipAcceptanceScript is Script {
    function run() external {
        ...
        for (uint256 i; i < targets.length; ++i) {
            payloads[i] = abi.encodeCall(IOwnershipAcceptance.acceptOwnership, ());
        }
        bytes32 salt = vm.envBytes32("GOVERNANCE_SALT");
        uint256 delay = vm.envOr("GOVERNANCE_DELAY", timelock.getMinDelay());
        vm.startBroadcast();
