...
contract SymbolicRewardMathHarness {
    function accruedIsZeroWhenIndexDoesNotAdvance(uint256 amount, uint256 index, uint256 userIndex) external pure returns (bool) {
        if (index > userIndex) return true;
        assert(RewardMath.userAccrued(amount, index, userIndex) == 0);
        return true;
    }

    function rewardIndexDoesNotDecrease(uint256 currentIndex, uint256 rewardRate, uint256 elapsed, uint256 totalStaked) external pure returns (bool) {
