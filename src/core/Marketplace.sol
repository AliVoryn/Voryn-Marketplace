...
contract Marketplace is Initializable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable, EIP712Upgradeable, IMarketplace, IUpgradeableSystem {
    ...
    bytes32 private constant REENTRANCY_STATUS_SLOT = keccak256("protocol.marketplace.reentrancy.status");
    ...
    modifier nonReentrant() {
        if (_reentrancyStatus() == REENTRANCY_ENTERED) revert ReentrancyGuardReentrantCall();
        _setReentrancyStatus(REENTRANCY_ENTERED);
        _;
        _setReentrancyStatus(REENTRANCY_NOT_ENTERED);
    }
    ...
    function _reentrancyStatus() private view returns (uint256 status) {
        bytes32 slot = REENTRANCY_STATUS_SLOT;
        assembly {
            status := sload(slot)
        }
    }
